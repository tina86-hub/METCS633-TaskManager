...
CREATE TABLE IF NOT EXISTS task  (
    task_id BIGSERIAL PRIMARY KEY,   -- task_id is a bigserial (auto-incrementing)
   ...
   CONSTRAINT fk_owner_id FOREIGN KEY (owner_id) REFERENCES "users" (user_id)  -- Foreign key constraint

);

-- Create the 'user_role' table if it does not already exist
CREATE TABLE IF NOT EXISTS user_role (
    user_id BIGINT NOT NULL,   -- user_id of type BIGINT, referencing the user table
    role_id INT NOT NULL,      -- role_id of type INT, referencing the role table
    CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES "users" (user_id),  -- Foreign key to the 'user' table
    CONSTRAINT fk_role FOREIGN KEY (role_id) REFERENCES role (role_id),    -- Foreign key to the 'role' table
